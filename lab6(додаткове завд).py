...
while True:
    ...
    # формування Y
    y = [[round(5.6 + 8.0 * x[0][j] + 4.8 * x[1][j] + 6.2 * x[2][j] + 5.9 * x[0][j] * x[0][j] + 1.0 * x[1][j] * x[1][j] +
                8.7 * x[2][j] * x[2][j] + 2.0 * x[0][j] * x[1][j] + 0.8 * x[0][j] * x[2][j] + 1.0 * x[1][j] * x[2][j] +
                3.0 * x[0][j] * x[1][j] * x[2][j] + randrange(0, 10) - 5, 2) for i in range(m)] for j in range(N)]
    ...
    if Gp < Gt:
        ...
        print("\nЧас визначення однорідності дисперсії (в секундах): ", timedelta(seconds=end_time - start_time).total_seconds())
